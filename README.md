...
│       │   ├── Navigation.js      # Sticky navbar with Auth0 login/logout
│       │   ├── OrderHistory.js    # Order history view
│       │   ├── ProductCard.js     # Individual product card
│       │   └── ProductList.js     # Product grid with search, filter & pagination
│       ├── App.css                # Dark SaaS glassmorphism styles
│       ├── App.js                 # Root component & API/state logic
│       ├── index.css              # Global fonts & CSS variables
│       └── index.js               # Entry point with Auth0Provider & BrowserRouter
├── infrastructure/                # Terraform infrastructure code
│   ├── api.tf                     # API Gateway configuration
│   ├── api_versioning.tf          # API versioning & JWT authorizer
...
│   ├── build_lambdas.ps1          # Packages each Lambda + utils.py into zip
│   └── seed_products.py           # Populates DynamoDB with 30 products
├── services/                      # AWS Lambda function source code
│   ├── cart_service.py            # Shopping cart management (Auth0 multi-tenant)
│   ├── order_processor.py         # Async fulfillment: inventory + SNS notification
...
├── *_guru.zip                     # Lambda deployment packages (generated by build_lambdas.ps1)
...
|-----------|---------|-----------|
| `services/` | AWS Lambda function source code | 6 Python services + `utils.py` handling logic |
...
| `scripts/` | Automation & Utilities | `build_lambdas.ps1`, `seed_products.py` |
| `tests/` | Unit test suite | 35 comprehensive tests with AWS mocking |
| Root | Project configuration & Artifacts | `*.zip` lambda deployment packages, docs |

### File Categories

- **Configuration Files**: `package.json`, `variables.tf`, `openapi-spec.json`
- **Deployment Packages**: `*_guru.zip` files generated in the project root by `build_lambdas.ps1`
- **Source Code**: `.py` files in `services/`, `tests/`, and `scripts/`
...
## Recent Updates (Late April 2026)

### ✅ **Auth0 Security Integration**
- **Frontend**: Integrated `@auth0/auth0-react` for secure login/logout flows.
- **Backend**: API Gateway now uses a JWT Authorizer to protect sensitive routes (`/cart`, `/order`).
- **Data Isolation**: Lambda functions dynamically extract the user's `sub` from the JWT claims to isolate DynamoDB data per tenant.

### ✅ **Performance & Pagination**
- **DB Seeding**: Extracted hardcoded product seeding into a standalone script (`scripts/seed_products.py`) to eliminate Lambda cold-start latency.
- **Cursor Pagination**: Upgraded DynamoDB scans to utilize `Limit` and `ExclusiveStartKey`.
- **Frontend Implementation**: Implemented a seamless "Load More" functionality for browsing products.
...
- **Build Script**: Created `build_lambdas.ps1` to intelligently bundle shared code into deployment zip files.

### ✅ **Async Order Fulfillment**
- **SQS Consumer**: Completely overhauled `order_processor.py` to handle asynchronous fulfillment.
- **Atomic Operations**: Processor securely deducts purchased items from master inventory via DynamoDB `UpdateExpression`.
- **Event-Driven Communications**: Processor publishes shipping confirmations to AWS SNS upon successful fulfillment.
...
**Core Features:**
- Auth0 login/logout with JWT token management
- Product browsing with cursor-based "Load More" pagination
- Category filtering and real-time search with API fallback
- Shopping cart management (per-user via Auth0)
- Order placement and order history viewing
- Payment processing

**Key Files:**
- `frontend/src/index.js` - Entry point wrapping app in `Auth0Provider` + `BrowserRouter`
- `frontend/src/App.js` - Root component: all API calls, state, and routing
- `frontend/src/App.css` - Dark SaaS glassmorphism design system
- `frontend/src/index.css` - Global CSS variables and Google Fonts (Inter, Outfit)
- `frontend/src/components/Navigation.js` - Sticky navbar with Auth0 login/logout
- `frontend/src/components/ProductList.js` - Product grid with filters and "Load More"
- `frontend/src/components/ProductCard.js` - Individual product card with hover effects
- `frontend/src/components/Cart.js` - Shopping cart view
- `frontend/src/components/OrderHistory.js` - Order history view
- `frontend/package.json` - Dependencies and scripts

**Auth0 Configuration (`index.js`):**
- Domain: `dev-sjgq3v6pvbgxs6mb.us.auth0.com`
- Audience: `https://api.mystore.com`
- Callback/Logout URL: `http://localhost:3000`
...
**Functions:**
1. **GET /products** → Returns paginated products (8 per page by default)
   - Query params: `limit` (default: 8), `lastEvaluatedKey` (cursor for next page)
2. **GET /products/{id}** → Returns specific product details

> **Note:** Product seeding is handled by the standalone `scripts/seed_products.py` script, not this Lambda.

**Database Operations:**
- Paginated scan of `products_table_guru` using DynamoDB `Limit` + `ExclusiveStartKey`
- Get item by ID
...
**Authentication:** Requires JWT Bearer token. User identity is extracted from the Auth0 `sub` claim — each user's cart is isolated by their unique ID.

**Database Operations:**
- Read from `products_table_guru` (stock check)
- Read/write `cart_table_guru` (keyed by Auth0 `user_id`)
...
```json
{
  ...
  "status": "created|shipped|paid",
  "user_id": "auth0|sub-claim"
}
```
...
**Functions:**
1. **Fulfillment Workflow** (triggered per SQS message)
   - Fetch full order record from `orders_table_guru`
   - **Deduct inventory**: Atomically decrement `stock_quantity` in `products_table_guru` for each ordered item using `UpdateExpression` with `ConditionExpression` to prevent negative stock
   - **Update order status**: Set order status to `shipped`
   - **Shipping notification**: Publish a "Your Order Has Shipped" message to SNS `order-notifications-guru` topic

**Database Operations:**
- Read `orders_table_guru` (fetch order)
- Update `products_table_guru` (atomic `stock_quantity` decrement per item)
- Update `orders_table_guru` (status → `shipped`)

**Messaging Integration:**
- **SNS:** Publishes shipping confirmation notification

**Message Processing:**
```
SQS Queue receives message from order_service
    ↓
order_processor Lambda invoked (batch size: 1)
    ↓
Fetch order → Deduct inventory → Update status → Send SNS
    ↓
...
**Items:** 30 products (seeded via `scripts/seed_products.py`)
...
**Access Patterns:**
- Paginated scan (using `Limit` + `ExclusiveStartKey`)
- Get product by ID (get item)
- Atomic `stock_quantity` decrement (via `order_processor` on fulfillment)
...
**User:** Auth0 `sub` claim (multi-tenant — each authenticated user has their own isolated cart)
...
```json
{
  "user_id": "auth0|64a8f...",
  "cart": [
    {"id": "prod_01", "name": "Laptop", "price": 49999, "quantity": 1},
    {"id": "prod_05", "name": "Headphones", "price": 1999, "quantity": 2}
  ]
}
```
...
1. **Real Payments:** Integrate Stripe/PayPal instead of random simulation
2. **Search Optimization:** Add DynamoDB Global Secondary Index for efficient search
3. **Analytics:** Send metrics to Amazon QuickSight dashboard
4. **Admin Dashboard:** Lambda + API for viewing all orders/payments
5. **Order Tracking:** Add order status history and events
6. **SMS Notifications:** Add SMS subscription to SNS for mobile alerts
